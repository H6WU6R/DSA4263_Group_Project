...
from dsa4263_group_project.feature_engineering import (
    engineer_temporal_features_leakage_free,  # NEW
    engineer_url_domain_features_leakage_free,  # NEW
...
# ========================================================================
# SECTION 3: FEATURE ENGINEERING (LEAKAGE-FREE)
# ========================================================================
print("\n" + "="*80)
print(" SECTION 3: FEATURE ENGINEERING (LEAKAGE-FREE)")
print("="*80)

# IMPORTANT: We need to split FIRST, then engineer features that depend on labels
print(f"\n[3.0] Splitting data BEFORE feature engineering...")
X_temp = df[['sender', 'receiver', 'date', 'label']].copy()
y = df['label']
...
# 3.1: Text meta-features (safe - no label dependency)
...
# 3.2: Temporal features (leakage-free - uses only training data)
df_with_features = engineer_temporal_features_leakage_free(
    df_with_features,
    train_idx,
    verbose=True
)

# 3.3: URL and domain features (leakage-free - uses only training data)
df_with_features = engineer_url_domain_features_leakage_free(
    df_with_features,
    train_idx,
    verbose=True
)
...
print(f"\n✅ Non-graph feature engineering complete (leakage-free)!")
print(f"  • Text Meta: {len(text_meta_features)} (no leakage)")
print(f"  • Temporal: {len(temporal_features)} (leakage-free)")
print(f"  • URL/Domain: {len(url_domain_features)} (leakage-free)")
print(f"  • Graph: Will be added next (leakage-free)")
